...
export function CategoriesPage() {
  ...
  const { data, isLoading, isFetching, isError } = useCategories({ page, limit, search })
  ...
  if (isError) {
    return <p className="text-sm text-destructive">Gagal memuat kategori.</p>
  }
    ...
    <div className="space-y-5 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Manajemen Kategori</h1>
          <p className="text-sm text-muted-foreground">Kelola data kategori produk</p>
        </div>
        <Button
      ...
      <DataTable
        ...
        isLoading={isLoading}
        isFetching={isFetching}
        emptyMessage={search ? "Data tidak ditemukan" : "Belum ada kategori"}
        emptyIcon={
          search ? (
